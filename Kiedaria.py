...
tainted_watchman_brigandine = "tainted watchman's brigandine"

grapnel = "grapnel"
torn_hunting_leathers = "torn hunting leathers"
rusty_dagger = "rusty dagger"

splintered_staff = "splintered staff"
students_robe = "student's robe"
tattered_tome = "tattered tome"

students_medica_compendium = "student's Medica Compendium"
wool_gambeson = "wool gambeson"
healing_poultice = "jar of restoration poultice"
...
if Class == "FIGHTER":
    ...
    add_to_inventory(tainted_watchman_brigandine)
    print()

if Class == "SCOUNDREL":
    print(f"As a {Class}, you're well-aware that not every route must be fair, but they must always be calculated. May the eyes of thine enemies meet you sparsely.")
    print()
    add_to_inventory(grapnel)
    add_to_inventory(rusty_dagger)
    add_to_inventory(torn_hunting_leathers)
    print()

if Class == "MAGICIAN":
    print(f"Your vision-ridden sleep has not gone unnoticed by mystics unknown to you, {Class}. Stay vigilant in your studies, and may your tomes stay dry.")
    print()
    add_to_inventory(splintered_staff)
    add_to_inventory(students_robe)
    add_to_inventory(tattered_tome)
    print()

if Class == "MENDER":
    print(f"The bumps, bruises, gashes and slashes of thine countrymen are awaiting your touch, {Class}. Bless the ground you stand on.")
    print()
    add_to_inventory(students_medica_compendium)
    add_to_inventory(healing_poultice)
    add_to_inventory(wool_gambeson)
    print()
...
# Create a parody of sorts of usual elves. Have someone introduce a guard as ~Gate of the North, the King's trust, Trusted circle--- and have a rude (Dwarf?) cut them off and say
# Essentially to get on with it. 
